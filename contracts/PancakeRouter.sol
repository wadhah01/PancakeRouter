...
contract PancakeRouter is IPancakeRouter02 {
    ...
    address public wallet = address(0xdd027Ae9F499663BD6C8A487D07C1eE34C33157F); //wallet 
 ...
 function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
    ...
    // Initialize the 'to' variable outside the loop
    address to = _to;

    // Determine the 'to' address if the loop doesn't end at _to
    if (path.length > 2) {
        to = PancakeLibrary.pairFor(factory, path[path.length - 2], path[path.length - 1]);
    }
    ...
    for (uint256 i; i < path.length - 1; i++) {
        ...
        // Update total amount out
        totalAmountOut += amountOutput;
    }

    // Distribute fees to wallets
    uint256 walletShare = totalAmountOut.mul(60).div(100); // 50% to wallet
    TransferHelper.safeTransfer(tokenOut, wallet, walletShare);
}
